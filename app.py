...
def calculate_od_interest(df, od_limit, annual_interest_rate, start_date=None, end_date=None):
    ...
    # 2. Continuous Date Range (Filling non-transaction days)
    ...
    # 5. Extract Actual Bank Charged Interest from Statement
    # Filtering transactions with Interest keywords
    int_mask = df['Narration'].str.contains(r'INT\.COLL|INT\. DEBIT|INTEREST CHARGED|OD INTEREST|INTEREST COLL', case=False, na=False)
    bank_int_df = df[int_mask & (df['Withdrawal Amt'] > 0)].copy()
    bank_int_df['Year_Month'] = bank_int_df['Parsed_Date'].dt.to_period('M')
        ...
        if raw_df.empty:
            ...
            if filtered_df.empty:
                ...
                with tab1:
                    ...
                    st.markdown("Yeh table dikhata hai ki system formula ke hisab se kitna interest banta hai aur bank ne kitna kaata hai:")
                    
                    # Styled Table Presentation
                    st.dataframe(
